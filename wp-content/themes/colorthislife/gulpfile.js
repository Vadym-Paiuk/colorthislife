  ...
  return gulp
    ...
    .pipe(
      rename(function (path) {
        path.dirname = "colorthislife/" + path.dirname;
      })
    )
    .pipe(zip("colorthislife.zip"))
    .pipe(gulp.dest(".."));
});

gulp.task("delete-zip-theme", () => del(["../colorthislife.zip"], { force: true }));

gulp.task("server", function () {
  browserSync.init({
    proxy: "http://colorthislife.loc",
    host: "colorthislife.loc",
    open: "external",
    notify: false,
  });
});
